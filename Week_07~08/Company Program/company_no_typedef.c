...
void print_information(struct Employee* employee, int size)
{
    FILE* ptr_file = fopen("notypedef_result.txt", "w+");
...
void pay(struct Employee** employee, int size)
{
...
void basic_pay(struct Employee** employee, int size)
{
    for (int i=0; i<size; i++)
    {
...
void additional_pay(struct Employee** employee, int size)
{
    for (int i=0; i<size; i++)
    {
        if (strcmp((*employee)[i].position, "과장") == 0) // 직책이 과장인 경우
...
void final_pay(struct Employee** employee, int size)
{
    for (int i=0; i<size; i++)
    {
